...
make_three_pannel_plot <- function(){
  ...
  covariates <- data_interventions[1:length(data_interventions$Country), c(1,2,3,4,5,6, 7, 8)]
  
  for(i in 1:length(countries)){    
    print(i)
    N_forecast = 7   
    N_ori <- length(dates[[i]])    
    N = N_ori + N_forecast   
    country <- countries[[i]]
    ...
    times_forecast <- times[N_ori] + 1:N_forecast  
    times[(N_ori+1):N] <- times_forecast[1:N_forecast]    
    reported_cases[[i]][(N_ori+1):N] <- rep(0,N_forecast)   
    deaths_by_country[[i]][(N_ori+1):N] <- rep(0,N_forecast)   
