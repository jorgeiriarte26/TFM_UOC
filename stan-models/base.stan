...
transformed parameters {
    ...
    matrix[N2,M] cum_prediction = rep_matrix(0,N2,M);
    matrix[N2,M] cum_E_deaths = rep_matrix(0,N2,M);
    for (m in 1:M){
      prediction[1:N0,m] = rep_vector(y[m],N0); // learn the number of cases in the first N0 days
        Rt[,m] = mu[m] * exp(covariate1[,m] * (-alpha[1]) + covariate2[,m] * (0.) +    
        covariate3[,m] * (-alpha[3])+ covariate4[,m] * (-alpha[4]) + covariate5[,m] * (0.) + 
        covariate6[,m] * (0.)); 
      for (i in (N0+1):N2) {
        convolution=0;
        for(j in 1:(i-1)) {
      ...
      {
      cum_prediction[1,m] = prediction[1,m];
      for (k in 2:N2){
        cum_prediction[k,m] = cum_prediction[k-1,m] + prediction[k,m];
      }
      }

      {
      cum_E_deaths[1,m] = E_deaths[1,m];
      for (k in 2:N2){
        cum_E_deaths[k,m] = cum_E_deaths[k-1,m] + E_deaths[k,m];
      }
      }
...
model {
  ...
  for(m in 1:M){
    for(i in EpidemicStart[m]:N[m]){
     ...
     // cum_deaths[i,m] ~ neg_binomial_2(cum_E_deaths[i,m],phi);   // to use with Iceland
    }
   }
}
